...
const listOfPlayers = document.querySelectorAll('.listOfPlayers');
const list1 = document.querySelector('#list1');
const list2 = document.querySelector('#list2');

list1.addEventListener('click', function (event) {
    const players = list1.querySelectorAll('li a');
    const actionTarget = document.querySelector('#actionTarget');
    players.forEach(function (e) {
        if (e.textContent == event.target.textContent) {
            actionTarget.textContent = e.textContent
        }
    })
})

list2.addEventListener('click', function (event) {
    const players = list2.querySelectorAll('li a');
    const votingTarget = document.querySelector('#votingTarget');
    players.forEach(function (e) {
        if (e.textContent == event.target.textContent) {
            votingTarget.textContent = e.textContent
        }
    })
})
